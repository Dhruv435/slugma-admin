  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.data?.message) {
        ...
        setMessage(`❌ Login failed: ${err.message || 'Network error'}`);
      }
    } finally {
            ...
            {isAdminAuthenticated ? (
              <nav className="w-full sm:w-auto">
                <ul className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-8 items-center">
                  <li><a href="/admin/home" className="hover:text-blue-200 text-lg font-medium py-2 px-3 rounded-md hover:bg-blue-700">Home</a></li>
                  <li><a href="/admin/products" className="hover:text-blue-200 text-lg font-medium py-2 px-3 rounded-md hover:bg-blue-700">Products</a></li>
                  <li><a href="/admin/orders" className="hover:text-blue-200 text-lg font-medium py-2 px-3 rounded-md hover:bg-blue-700">Orders</a></li>
                  <li><a href="/admin/users" className="hover:text-blue-200 text-lg font-medium py-2 px-3 rounded-md hover:bg-blue-700">Users</a></li>
                  <li>
                    <button
                      onClick={handleAdminLogout}
                      className="bg-red-600 hover:bg-red-700 text-white font-semibold py-2.5 px-5 rounded-lg flex items-center shadow-lg transform hover:scale-105 active:scale-95"
                    >
              ...
              <a href="/admin/login" className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2.5 px-5 rounded-lg shadow-lg transform hover:scale-105 active:scale-95">
                Login
              </a>
            )}
          </div>
        ...
        <main className="flex-grow p-6 sm:p-8">
          <Routes>
            <Route
              path="/admin/login"
              element={
                isAdminAuthenticated ? <Navigate to="/admin/home" replace /> : <LoginForm />
              }
            />
            <Route
              path="/admin/*"
            ...
            <Route
              path="/admin"
              element={isAdminAuthenticated ? <Navigate to="/admin/home" replace /> : <Navigate to="/admin/login" replace />}
            />
          </Routes>
        </main>
      </div>
